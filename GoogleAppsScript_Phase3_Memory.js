/* ================================================================
   ...
   Versi: 5.0 (Dengan Ingatan / Conversation Memory)
   ================================================================ */
...
function doPost(e) {
  ...
  try {
    ...
    from    = data.from || "-";
    history = data.history || []; // Menerima ingatan dari Node.js
    ...
    if (intent === "FINANCE") {
       ...
       if (!isOwner) return respond_("Akses Ditolak.");
...
function routerAgent(pesan, isOwner, history) {
  let sys = `You are an Intent Classifier Agent. Read the conversation history to understand the context of the user's latest message.
Rules:
1. FINANCE: User is logging expenses or income.
2. OPS: User is actively ordering, checking out, OR providing shipping address/location (e.g. "ke bali", "alamat saya di...").
3. ADMIN: Command to a virtual assistant.
4. CS_SALES: General question, asking for product price, chatting.
...
Output ONLY pure JSON format: {"intent": "FINANCE" | "OPS" | "ADMIN" | "CS_SALES"}`;
  ...
  if (finalIntent === "ADMIN" && !isOwner) finalIntent = "CS_SALES";
  return finalIntent;
}
...
function financeAgent(ss, pesan, session, from, history) {
  const sys = "You are an Indonesian financial extractor. Read the conversation. Output JSON array 'data'. Format: {\"data\":[{\"cat\":\"string\",\"amt\":number,\"type\":\"Masuk|Keluar\",\"rek\":\"string\"}]}";
...
[KATALOG]: ${katalog}
[FAQ]: ${faq}
3. Remember previous context. Do not make up info.`;
...
Read the conversation history. If the user mentions an item, output JSON:
{"sku_ditemukan": "SKU", "jumlah": 1, "reply": "Pesan ramah konfirmasi pesanan dan minta alamat lengkap."}

If the user provides an address (even just a city name), acknowledge it and output JSON:
{"reply": "Pesan ramah pesanan akan dikirim ke lokasi tersebut, berikan nomor rekening BCA 12345."}

Always output pure JSON.
[KATALOG]: ${katalog}`;

  const result = panggilGroq_(sys, history, 300);
...
Address the owner respectfully. Read the conversation history to understand context.`;
  return panggilGroqChat_(sys, history);
}
...
function panggilGroq_(systemPrompt, historyArray, maxTokens) {
  // historyArray sudah berisi [{role:'user'...}, {role:'assistant'...}, ..., {role:'user', content: pesanTerakhir}]
